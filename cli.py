...
def t(ctx,recipient_address, amount):
    ...
    my_ip = socket.gethostbyname(socket.gethostname())      # get current machine's IP to build the url
    url = "http://" + str(my_ip) + ":" + str(port) + '/cli_transaction'                 # build correct url to access the corresponding endpoint to the correct REST API
    click.echo("Transaction pending...")
    req = requests.post(url, data={"address" : recipient_address, "amount": amount})    # send post request and get the response
