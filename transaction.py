...
class Transaction:
    def __init__(self, sender_address, sender_private_key, recipient_address, value):
        ...
        rand_string = Crypto.Random.get_random_bytes(
            N
        ...
        message_to_hash = (
            ...
            + str(rand_string)
            + str(time.time())
        )  # transaction hash will be created from this message
        self.transaction_id = SHA.new(
            message_to_hash.encode()
        ).hexdigest()  # encode() converts the string into bytes to be acceptable by hash function
        # create hash object and get encoded hash in hexadecimal format
        self.inputs = (
            []
        )  # input list initially empty, will be updated by create_transaction()
        self.outputs = (
            []
        )  # output list initially empty, will be updated by create_transaction()

    def to_dict(
        self, add_signature=False
    ):  # function to convert transaction from object to dictionary
        ...
        if (
            not add_signature
        ):  # do not add transaction signature (used for calculating signature)
            transaction_dict = {
    ...
    def sign_transaction(self):  # function to calculate transaction signature
        ...
        transaction_to_sign = str(
            self.to_dict()
        )  # convert transaction to dictionary --> get all its fields
        hashed_transaction = SHA.new(
            transaction_to_sign.encode("utf-8")
