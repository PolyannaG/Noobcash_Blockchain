                ...
                for item in processed_block.listOfTransactions:
                    try:
                        for (
                            ...
                            item.outputs
                        ):  # inputs unspent, time to add outputs to NBCs list
                            for (
                                item
                            ) in (
                                node_instance.ring
                            ):  # find id of node whose wallet will get the NBCs
...
if __name__ == "__main__":
    ...
    )  # number of nodes in rin
    parser.add_argument(
        "-b", "--bootstrap", default="127.0.0.1", type=str, help="bootstrap address"
