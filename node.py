...
class Node:
    def __init__(self):
        ...
        self.time_to_append = (
            []
        )  # list to hold the time it has taken between the creation of a block and the moment it is appended to the blockchain
        ...
        self.locks = {
            "NBCs": NBCs_lock,
            "valid_trans": valid_transactions_lock,
            "cur_block": cur_block_lock,
            "chain": chain_lock,
            "conf": conflict_lock,
        }
        ...
        self.current_id_count = (
            0  # used by bootstrap to count how many nodes it has registered
        )
        ...
        self.ring = (
            []
        )  # here we store information for every node: its id, its contact info (ip:port), its wallet address, its public key and its balance
        self.current_block = (
            None  # the block at which new transactions are beeing appended
        )
        ...
        self.get_back = (
            set()
        )  # set to keep outputs that are the money the node will get back from the transactions
        ...
        self.transactions_done = (
            []
        )  # transactions the node created and are added to blockchain
    ...
    def register_node_to_ring(self, public_key, address, contact):
        ...
        try:
            node_info = {
                "node_id": self.current_id_count + 1,
                "contact": contact,
                "address": address,
    ...
    def create_transaction(self, sender, receiver, amount, signature=None):
        if sender == "0":  # transaction for genesis block
            new_transaction = Transaction(
                sender, self.wallet.private_key, receiver, amount
            )
            ...
            new_transaction.outputs = [
                (
                    ...
                    binascii.b2a_hex(receiver).decode("utf-8"),
                    amount,
                )
            ]
            ...
            for (
                node_item
            ) in self.ring:  # check that sender node is indeed part of the ring
            ...
            for (
                node_item
            ) in self.ring:  # check that receiver node is indeed part of the ring
            ...
            if sender_id == None:
                ...
                sender_id != self.id
            ):  # check that the node is indeed the current one (for safety, should always be true)
                ...
                total = self.wallet.balance(
                    self.NBCs[sender_id]
                )  # calculate node's balance its from UTXOs
                for (
                    item
                ) in (
                    self.used_nbcs
                ):  # true balance is the balance calculated minus outputs of pending transactions
                    total -= item[3]
                ...
                if total < amount:  # total not enough
                    ...
                    if temp_length == 0:  # no pending transactions
                        ...
                        return (
                            "Not enough NBCs for the specified transaction.",
                            400,
                            None,
                        )
                        ...
                        while True:
                            ...
                            if (
                                total + sum_ < amount
                            ):  # if balance plus total is not enough for the transaction, do not do it
                                self.locks["NBCs"].release()
                                return (
                                    "Not enough NBCs for the specified transaction.",
                                    400,
                            ...
                            if (
                                temp_length == len(self.pending_transaction_ids)
                                and total < amount
                            ):  # if the number of penfing transactions has not changes and the true balance is not enough, sleep and check later
                                self.locks["NBCs"].release()
                            ...
                            if (
                                total < amount
                            ):  # if length of pending transactions has changes and total still is not enough:
                                self.locks["NBCs"].release()
                                temp_length = len(self.pending_transaction_ids)
                                if (
                                    temp_length == 0
                                ):  # if there are no more pending transactions:
                                    return (
                                        "Not enough NBCs for the specified transaction.",
                                        400,
                                        None,
                ...
                try:
                    ...
                    for item in self.NBCs[
                        sender_id
                    ]:  # find the previous outputs the money will come from
                    ...
                    difference = (
                        cur_sum - amount
                    )  # calculate money to get back from transaction

                    new_transaction = Transaction(
                        sender, self.wallet.private_key, receiver, amount
                    )  # create the trascaction
                    if difference != 0:
                        out = (
                            str(uuid.uuid1()),
                            new_transaction.transaction_id,
                            sender,
                            difference,
                        )  # the output with money to get back
                        outputs.append(out)
                    ...
                    out2 = (
                        ...
                        receiver,
                        amount,
                    )  # the ouptut with money to be given to receiver
                    outputs.append(out2)
                    ...
                    self.pending_transaction_ids.add(
                        new_transaction.transaction_id
                    )  # add transaction to pending
                    self.transactions_created.append(
                        (new_transaction.transaction_id, new_transaction.amount)
                    )  #  add transaction id and amount to created
    ...
    def transaction_sending(*args):  # function to senf transaction to specified node
        ...
        try:
            res = requests.post(
                "{}/transactions/receive".format(node_["contact"]),
                json=trans_to_broadcast,
            )
        except:
            True

    async def broadcast_transaction(
        self, transaction, to_all=True
    ):  # funtion to broadcast transaction to all nodes
        trans_to_broadcast = transaction.to_dict(
            True
        )  # get transaction in dictionary form (sendable)
        if to_all:  # broadcast to all nodes
            for node_ in self.ring:
                sent_thread = threading.Thread(
                    target=self.transaction_sending, args=(node_, trans_to_broadcast)
                )
                sent_thread.start()
    ...
    def validate_transaction(
        self, transaction, from_resolve_conflict=False
    ):  # function to validate received transaction
        sender_address = a2b_hex(transaction.sender_address)
        key = RSA.importKey(
            sender_address
        )  # public key of sender is the sender's address
        signature = a2b_hex(transaction.signature)
        trans_to_hash = str(transaction.to_dict())
        hashed_transaction = SHA.new(
            trans_to_hash.encode("utf-8")
        )  # hash transaction to verify its signature

        try:
            # signature verification
            is_verified = PKCS1_v1_5.new(key).verify(
                hashed_transaction, signature
            ...
            if is_verified:  # transaction signature validated, now we check UTXOs
                ...
                for input in transaction.inputs:  # check that every input is unspent
                    ...
                    if (
                        not found_utxo
                    ):  # input is not unspent output, transaction is not valid
    ...
    def update_nbcs(
        self, transaction, from_resolve_conflict=False
    ):  # function to update UTXO list (self.NBCs), according to a new transaction
        if not from_resolve_conflict:
            self.locks["NBCs"].acquire()
        sender_id = self.id
    ...
    async def send_blockchain(
        self,
    ):  # function for bootstrap to send blockchain_to_nodes
        block_list = []
        ...
        for node_ in self.ring[1:]:  # send blockchain to all nodes in ring
            t = threading.Thread(
                target=self.persistent_sending_blockchain_to_nodes,
                args=(node_, data, num),
            )  # new thread for each node, call insinde function for the rest of the functionality
            t.start()
    ...
    def persistent_sending_blockchain_to_nodes(self, *args):
        ...
        while not sent:  # try to send blockchain until succesful
            try:
                res = requests.post(
                    "{}/blockchain/get".format(node_["contact"]), json=data
                )
                if res.status_code == 200:  # blockchain sent
                    ...
                    if (
                        num[0] == len(self.ring) - 1
                    ):  # if all nodes have received the ring, create final transaction for the last node and broadcast it
                        print("all nodes have received blockchain")

                        message, error_code, trans = self.create_transaction(
                            self.wallet.address, self.ring[-1]["address"], 100
                        )
                        threading.Thread(
                            target=asyncio.run,
                            args=(self.broadcast_transaction(trans, False),),
                        ).start()
                        if error_code != 200:
    ...
    def persistent_sending_ring_to_nodes(
        self, *args
    ):  # function to send ring to all nodes, and afterwards call function to send blockchain
        node_, data, num = args
        sent = False
        while not sent:  # try to send ring until succesful
            try:
                ...
                if res.status_code == 200:  # ring sent
                    ...
                    if num[0] == len(
                        self.ring
                    ):  # if all nodes have received the ring, time to broadcast blockchain
                        print("all nodes have reiced ring")

                        # all nodes have received ring, time to broadcast blockchain
                        t = threading.Thread(
                            target=asyncio.run, args=(self.send_blockchain(),)
                        )
                        t.start()
                else:  # error in sending
                    print(
                        "error sending ring to node:",
                        node_["node_id"],
                        res.json()["message"],
                    )
                    sent = False
    ...
    def send_data_to_nodes_give_blockchain(self):
        ...
        for node_ in self.ring:  # send ring to all nodes in ring
            t = threading.Thread(
                target=self.persistent_sending_ring_to_nodes, args=(node_, data, num)
            )  # new thread for each node, call insinde function for the rest of the functionality
            t.start()

    def add_transaction_to_block(
        self, transaction, capacity=None
    ):  # function to add a validated transaction to a block to be mined
        self.locks["chain"].acquire()
        self.locks["cur_block"].acquire()
        is_first = False  # variable to hold whether transaction added is the first of the block
        if (
            self.current_block == None
        ):  # there is no block with some transactions already, create new one
            is_first = True
            ...
            if self.chain == []:  # case of genesis block
                ...
                self.current_block = self.create_new_block(
                    self.chain[-1].index + 1, self.chain[-1].hash, None, capacity
                )
            else:
                self.current_block = self.create_new_block(
                    ...
                    None,
                    self.block_capacity,
                )  # usual case, create next current block following the last of the blockchain

        self.locks["chain"].release()
        self.current_block.listOfTransactions.append(
            transaction
        )  # add transaction to list of transactions in the block
        if (
            len(self.current_block.listOfTransactions) == self.current_block.capacity
            or time.time() - self.current_block.timestamp > 5
        ):  # if block is full or the block has timed out, time to mine
            try:
                t = threading.Thread(
                    target=asyncio.run,
                    args=(self.mine_block(copy.copy(self.current_block)),),
                )
                t.start()
            ...
            if is_first:
                t = threading.Thread(
                    target=asyncio.run, args=(self.check_for_mine(),)
                )  # create thread to watch for block timeout
                t.start()
        return

    async def check_for_mine(
        self,
    ):  # function to check whether the current block to which transactions are beeing added has timed out
        while True:
            # print('checking for mine')
            self.locks["cur_block"].acquire()
            if (
                self.current_block != None
                and time.time() - self.current_block.timestamp > 5
                and len(self.current_block.listOfTransactions) != 0
            ):  # if block is not empty and has timed out, time to mine
                t = threading.Thread(
                    target=asyncio.run,
                    args=(self.mine_block(copy.copy(self.current_block)),),
                )
    ...
    async def mine_block(self, block_to_mine):  # function to mine a block
        if (
            len(block_to_mine.listOfTransactions) == 0
        ):  # if block has no transactions stop mining
            return

        block_to_mine.nonce = random.randint(
            0, 10000000000000
        )  # set initial nonce. random choice
        while not block_to_mine.myHash().startswith(
            "0" * block_to_mine.difficulty
        ):  # keep trying hashing with a different once until number of zeros specified is reached
            if (
                len(block_to_mine.listOfTransactions) == 0
            ):  # if block has no transactions, return
                return
            block_to_mine.nonce = random.randint(
                0, 1000000000000000
            )  # pick a new nonce
            ...
            self.locks["chain"].acquire()
            if (
                self.chain != [] and self.chain[-1].index >= block_to_mine.index
            ):  # check if new block has been added to blockchain
                for block_item in self.chain[
                    block_to_mine.index :
                ]:  # for all new added blocks
                    for (
                        trans
                    ) in (
                        block_item.listOfTransactions
                    ):  # for all transactions of the blocks
                        i = 0
                        while i < len(block_to_mine.listOfTransactions):
                            # check if they exist in the block beeing mined
                            if (
                                trans.transaction_id
                                == block_to_mine.listOfTransactions[i].transaction_id
                            ):
                                block_to_mine.listOfTransactions.pop(
                                    i
                                )  # if yes, remove them from block being mined
                                i -= 1
                                if (
                                    len(block_to_mine.listOfTransactions) == 0
                                ):  # if no more transactions in block, stop mining
                ...
                block_to_mine.previousHash = self.chain[
                    -1
                ].hash  # correct previous hash
                block_to_mine.nonce = random.randint(
                    0, 10000000000000
                )  # pick new random nonce
        ...
        if len(self.ring) < self.node_number:
            ...
            self.chain.append(
                block_to_mine
            )  # add block straight to chain, it will be shared with the nodes later
            # should the chain object the Blockchain or a list?

            for (
                trans
            ) in (
                block_to_mine.listOfTransactions
            ):  # remove from pending transactions, remove outputs from used_nbcs etc.
        ...
        if len(self.ring) == self.node_number:
            ...
            threading.Thread(
                target=asyncio.run, args=(self.broadcast_block(block_to_mine),)
            ).start()
    ...
    def block_sending(*args):  # function to send block to specified node
        ...
        try:
            res = requests.post(
                "{}/blocks/receive".format(node_["contact"]), json=block_to_broadcast
            )
        except:
            True
    ...
    async def broadcast_block(self, block):  # function to broadcast block to all nodes
        ...
        for node_ in self.ring:
            sent_thread = threading.Thread(
                target=self.block_sending, args=(node_, block_to_broadcast)
            )
            sent_thread.start()
        return

    def update_ring_amounts(
        self,
    ):  # function to update balances of nodes in self.ring, based on self.NBCs
        for i in range(0, len(self.ring)):
            self.ring[i]["balance"] = self.wallet.balance(self.NBCs[i])

    def validate_block(
        self, block, from_resolve_coflict=False
    ):  # function to validate a received block
        load_dotenv()  # load environment variables
        if block.myHash().startswith(
            "0" * int(os.getenv("DIFFICULTY"))
        ):  # check that block hash is correct (has right number of zeros in the beginning)
            # block hash correct
            if not from_resolve_coflict:
                self.locks["chain"].acquire()

            if (
                len(self.chain) >= 1 and block.previousHash != self.chain[-1].hash
            ):  # check that block is the expected, so previousHash is the hash of the previous block in blockchain
                try:
                    self.locks["chain"].release()
                    self.locks["NBCs"].release()
                except:
                ...
                if (
                    block.index < self.chain[-1].index
                    or block.hash == self.chain[-1].hash
                ):  # block earlier in chain, conflict already solved, no need to solve again
                ...
                index_to_ask = max(
                    block.index - 10, 0
                )  # index of block from which and afterwards the chosen node will send chain
                t = threading.Thread(
                    target=self.resolve_conflicts, args=(index_to_ask,)
                )
                t.start()
                t.join()
            ...
            for (
                trans
            ) in block.listOfTransactions:  # check that all transactions are valid
            ...
            for (
                ...
                block.listOfTransactions
            ):  # all transactions valid, block will be appended to chain, so update NBCs
            ...
            # print('hash not ok')
            threading.Thread(
                target=asyncio.run, args=(self.resolve_conflicts(),)
            ).start()
            return False
    ...
    def restore_nbcs(
        self, transaction
    ):  # function to restore NBCs to the way they were before given transaction was accepted
        ...
        for (
            ...
            transaction.inputs
        ):  # add all inputs of transaction back as unspent outputs
            # print(input)
        ...
        for (
            output
        ) in transaction.outputs:  # iremove outputs of transaction from unspent outputs
    ...
    def resolve_conflicts(self, index):  # function to resolve conflict in chain
        ...
        if max_node != self.ring[self.id]["contact"]:
            while (
                True
            ):  # ask chosen node to send blockchain until blockchain is successfully received
                res = requests.get(
                    max_node + "/blockchain/get", params={"index": index}
                )  # index after which the sub-chain will be sent is specified
                if res.status_code == 200:
                    res = res.json()
                    break
            try:
                ...
                while (
                    k < len(chain)
                    and j < len(self.chain)
                    and chain[k]["hash"] == self.chain[j].hash
                ):
                    k += 1
                    j += 1
                ...
                if j < len(self.chain):
                    thrown_away = self.chain[
                        j:
                    ]  # get part of old chain to be thrown away
                    # print(thrown_away)
                    ...
                    for (
                        ...
                        thrown_away
                    ):  # restore NBCs for all transactions in blocks to be thrown away
                ...
                for i in range(k, len(chain)):
                    processed_block = self.process_block(
                        chain[i]
                    )  # convert to object from dictionary
                    ...
                    if i == 0:  # genesis block, not validated. just update NBCs
                        ...
                        ):  # block not valid, this and all following will not be added to blockchain, start proccess of resolving conflict again
                    ...
                    self.chain.append(
                        processed_block
                    )  # block valid, append block to chain

                    for (
                        trans
                    ) in (
                        processed_block.listOfTransactions
                    ):  # remove transactions from pending, update used_nbcs etc.
                        if trans.transaction_id in self.pending_transaction_ids:
                            for input_ in trans.inputs:
                                if input_ in self.used_nbcs:
                                    self.used_nbcs.remove(input_)
    ...
    def process_transaction(
        self, item
    ):  # function to convert transaction from dictionary form to object
        try:
    ...
    def process_block(
        self, data
    ):  # function to convert block from dictionary form to object
        try:
    ...
    def send_blockchain_resolve_conflict(
        self, index
    ):  # function to send blockchain to node
        block_list = []
        self.locks["chain"].acquire()
        for i in range(
            int(index), len(self.chain)
        ):  # create list of blocks in sendable form
            block_list.append(self.chain[i].to_dict(True))
        self.locks["chain"].release()
        return block_list
    ...
    def view_transaction(
        self,
    ):  # function to get the last valid block of the current blockchain (for cli and frontend)
        my_chain = self.chain
        if my_chain == []:  # if the current blockchain is empty return None
            return None
        else:  # else
            last_valid_block = my_chain[
                -1
            ]  # get the last valid block of the blockchain
