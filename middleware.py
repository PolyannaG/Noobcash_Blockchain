...
def get_contact(id):                                                    # function to get the address of a node, given its ID 
    my_ip = socket.gethostbyname(socket.gethostname())                  # get IP address for the current machine to build correct url
...
def homepage(myid):                              
    node_id = int(myid)
    address = get_contact(node_id)                      # get the contact of the correct node in order to access the correct endpoint in the correct rest api

    if address == None:                                 # If the node id is not part of the ring
        return render_template("no_node.html")          # render no_node page to inform the user that the node ID he typed in the url does not exist 

    url = str(address) + "/homepage"                    # build url
    mydata = json.loads((requests.get(url)).text)       # get response from the request we sent and make it a json object (from string)

    arg = []
    arg.append(mydata['id'])                            # create list with response data in order to be able to pass it in the render_template data

    return render_template("homepage.html", data = arg) # render homepage with the correct data we collected from the above request
...
@app.route("/transaction/nodeID/<myid>", methods = ['GET', 'POST'])     # Transaction page -> create a transaction
def create_transaction_front(myid):
    ...
    if request.method == 'POST':                                        # if POST METHOD, "Create Transaction" button has been pushed, we need to create a transaction with the data collected from the form in the frontend
        receiver_address = request.form.get("address")                  # get receiver address from the frontend's input form
        amount = int(request.form.get("amount"))                        # get amount of coins to send from frontend's input form
        post_data = {"address" : receiver_address, "amount" : amount}   # make them json object in order to be able to pass them in post request
        
        mydata = json.loads((requests.post(url, json = post_data)).text)
       
        arg.append(mydata['id'])
        flash(mydata["message"])                                        # flash (show to html frontend page) the return message we got from the endpoint
...
@app.route("/view/nodeID/<myid>")                       # View Page -> see the last valid block's transactions
def view(myid):
    node_id = int(myid)
    address = get_contact(node_id)
    ...
    if mydata["none"] == True:                          # if no valid block in the blockchain
        ...
        arg.append(time.strftime("%a, %d %b %Y %I:%M:%S %p %Z", time.localtime(mydata["t"])))       # get block creation time in sec and change it in date-time format so that it can be displayed in frontend
        arg.append(mydata["transactions"])

        return render_template("view.html", data = arg)


@app.route("/balance/nodeID/<myid>")                    # Balance page -> see the wallet balance of the node that currently uses the frontend interface
def balance(myid):
    node_id = int(myid)
    address = get_contact(node_id)
...
@app.route("/help/nodeID/<myid>")                       # Help page -> see explanatory information about all the other pages in the frontend interface
def help(myid):     
    node_id = int(myid)
    address = get_contact(node_id)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--address', default='127.0.0.1', type=str, help='machine address')
